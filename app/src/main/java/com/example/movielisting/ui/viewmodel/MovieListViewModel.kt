...
class MovieListViewModel @Inject constructor(movieApiService: MovieApiService) : ViewModel() {
    ...
    private val upcomingdMoviesLD = MutableLiveData<Pair<ListType, List<MovieEntity>>>()
    ...
    fun connected(events: Observable<Pair<ListType, Long>>) {
        events.doOnNext { Log.e(TAG, "${it.first} , ${it.second}") }
            .flatMap { checkData(it.second, it.first) }
            .observeOn(AndroidSchedulers.mainThread())
            .subscribe {
                upcomingdMoviesLD.postValue(it.first to it.second)
            }
        ...
        subscriptions.add(
            checkData(1, UPCOMING)
                .doOnNext { Log.e(TAG, "next $UPCOMING") }
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe({
                    upcomingdMoviesLD.postValue(it)
                }, {
    ...
    fun getScrollResponses(): LiveData<Pair<ListType, List<MovieEntity>>> {
        return upcomingdMoviesLD
    }
